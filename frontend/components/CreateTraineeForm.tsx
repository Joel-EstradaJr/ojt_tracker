    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" style={{ maxWidth: 560, maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
            ...
            <div className="form-group">
              ...
              <select value={suffix} onChange={(e) => setSuffix(e.target.value)}>
                {SUFFIX_OPTIONS.map((opt) => (<option key={opt} value={opt}>{opt || "\u2014 None \u2014"}</option>))}
              </select>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
            <div className="form-group">
              <label>Email * {emailVerified && <span style={{ color: "var(--success-text)", fontSize: "0.8rem" }}>{"\u2713"} Verified</span>}</label>
              <div style={{ display: "flex", gap: "0.35rem" }}>
                <input type="email" value={email} onChange={(e) => handleEmailChange(e.target.value)} placeholder="juan@email.com" style={{ flex: 1, ...(emailVerified ? { borderColor: "var(--success)" } : {}) }} disabled={emailVerified} />
                {!emailVerified && (
