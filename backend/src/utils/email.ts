...
/**
 * Send a temporary password to a new user account created by an admin.
 * The user must change this password on first login.
 */
export async function sendTemporaryPassword(to: string, tempPassword: string, displayName: string): Promise<void> {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
      <h2 style="color: #1e293b; margin-bottom: 8px;">OJT Progress Tracker</h2>
      <p style="color: #475569; font-size: 14px;">Hi <strong>${displayName}</strong>,</p>
      <p style="color: #475569; font-size: 14px;">
        An account has been created for you. Use the temporary password below to log in
        for the first time. You will be required to set a new password immediately.
      </p>
      <div style="text-align: center; margin: 24px 0;">
        <span style="display: inline-block; font-size: 20px; font-weight: 700; letter-spacing: 2px; padding: 12px 24px; background: #f1f5f9; border-radius: 8px; color: #0f172a;">
          ${tempPassword}
        </span>
      </div>
      <p style="color: #475569; font-size: 14px;">
        <strong>Important:</strong> This temporary password can only be used once.
  ...
  await transporter.sendMail({
    from: `"OJT Progress Tracker" <${process.env.SMTP_EMAIL}>`,
    to,
    subject: "Your Account Has Been Created — OJT Progress Tracker",
    html,
  });
}
